...
#: Parsed HTML, keyed by the file's modification time. Rendering 12 KB of
#: Markdown per request would be wasteful for a page that changes about never,
#: and keying on mtime means an edit still shows up without a restart.
_rendered: tuple[float, str] | None = None
...
def _whitepaper_html() -> str:
    """Render docs/whitepaper.md, reusing the last parse until the file moves."""
    global _rendered

    try:
        stamp = WHITEPAPER_PATH.stat().st_mtime
    except OSError as exc:
        raise Http404("The whitepaper is not available.") from exc

    if _rendered is None or _rendered[0] != stamp:
        import markdown

        html = markdown.markdown(
            WHITEPAPER_PATH.read_text(encoding="utf-8"),
...
def whitepaper(request):
    ...
    return render(
        ...
        {"body": mark_safe(_whitepaper_html())},  # noqa: S308 - repo file, see above
    )
